...
bool CheckFeatureVersion(const std::shared_ptr<SDMController>& controller, feature_ver_sw feature) {
    sdm_feature_version version;
    uint32_t flags = 0;

    if (controller->get_feature_version(feature, &version, &flags) != 0) {
        return false;
    }
