...
bool DisplayModes::isSupported() {
    ...
    if (!utils::CheckFeatureVersion(controller_, FEATURE_VER_SW_SAVEMODES_API) ||
        controller_->get_num_display_modes(0, 0, &count, &flags) != 0) {
        return 0;
    }
    return count > 0;
}
...
std::vector<DisplayMode> DisplayModes::getDisplayModesQDCM() {
    ...
    if (controller_->get_num_display_modes(0, 0, &count, &flags) != 0 || count == 0) {
        return modes;
    }

    sdm_disp_mode tmp[count];

    if (controller_->get_display_modes(0, 0, tmp, count, &flags) == 0) {
...
int32_t DisplayModes::getDefaultDisplayModeIdQDCM() {
    ...
    if (controller_->get_default_display_mode(0, &id, &flags) != 0) {
        id = -1;
    }
...
bool DisplayModes::setModeState(int32_t mode_id, bool on) {
    ...
    return controller_->set_active_display_mode(0, id, 0) == 0;
}
...
Return<bool> DisplayModes::setDisplayMode(int32_t mode_id, bool make_default) {
    ...
    if (make_default) {
        ...
        if (controller_->set_default_display_mode(0, mode_id, 0) != 0) {
            return false;
        }
    }
